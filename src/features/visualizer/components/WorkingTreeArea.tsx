...
interface WorkingTreeAreaProps {
  files: FileStatus[];
  isLoading?: boolean;
  isError?: boolean;
}

export function WorkingTreeArea({ files, isLoading, isError }: WorkingTreeAreaProps) {
  return (
    <div className="flex flex-col h-full min-w-[220px] rounded-[var(--radius-lg)] border border-[var(--color-border)] bg-[var(--color-bg-subtle)] overflow-hidden">
      <div className="flex items-center justify-between px-4 py-3 border-b border-[var(--color-border)] bg-[var(--color-bg)]">
        <span className="text-sm font-semibold text-[var(--color-text-primary)]">Working Tree</span>
        {!isLoading && !isError && (
      ...
      <div className="flex-1 p-3 flex flex-col gap-2 overflow-y-auto">
        ...
        {!isLoading && !isError && files.length === 0 && (
          <div className="flex items-center justify-center h-full">
            <p className="text-sm text-[var(--color-text-muted)]">変更なし（クリーンな状態）</p>
          </div>
        )}
